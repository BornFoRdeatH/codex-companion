...
The 1.3 Extension Platform renderer controls are available but disabled by default in 1.3.12.
...
Version 1.3.12 keeps public config `schema_version = 1`, uses internal SQLite schema v6, and adds
...
Version 1.3.11 restores the Extension Platform runtime with widgets and extension controls disabled
...
Version 1.3.12 resets the local feature-toggle namespace again so all previously enabled widgets
...
Composer footer widget/action placement is temporarily disabled; any persisted `composer_footer`
placement falls back to `dock` to avoid renderer freezes while the footer integration is hardened.
