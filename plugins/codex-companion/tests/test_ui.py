...
class UiTests(unittest.TestCase):
    ...
    def test_launcher_uses_stable_version_family_and_ignores_empty_cache_entries(self) -> None:
        ...
        self.assertIn("manifest_version", source)
        self.assertIn('".codex-plugin" / "plugin.json"', source)
    ...
    def test_runtime_uses_dynamic_windows_remaining_and_native_style(self) -> None:
        ...
        self.assertIn("builtinAction:true", source)
        self.assertIn('actionsForPlacement("composer_footer")', source)
        ...
        self.assertIn("handoffTurnContainer", source)
        self.assertIn("maybeCaptureHandoffBase", source)
        self.assertIn('container?.closest?.("[data-turn-key]")', source)
        ...
        self.assertIn("gitSummaryUpdate", source)
        self.assertIn("state.handoffPreview.markdown", source)
        ...
        self.assertIn('row.style.display="none"', source)
        self.assertIn('state.fullScanRequested=true;scheduleScan(true,[anchor])', source)
        self.assertIn('gate.style.maxWidth="100%"', source)
        self.assertIn('codex-companion-composer-actions', source)
        self.assertNotIn('updateComposerClearance', source)
        self.assertIn('codexCompanionVisibleTurns', source)
        self.assertIn('renderWidgetSettingsBase', source)
        self.assertIn('settingsSurface', source)
        self.assertIn('settingsSummary', source)
        self.assertIn('renderStableSettings', source)
        self.assertIn('settingsWidgetsSection', source)
        self.assertIn('settingsFastSection', source)
        self.assertIn('settingsFocusSection', source)
        self.assertIn('settingsPrivacySection', source)
        self.assertIn('settingsWidgetsBody', source)
        self.assertIn('settingsSection', source)
        self.assertIn('content.replaceChildren()', source)
        self.assertIn('configEditor', source)
        self.assertIn('Recent turns visible', source)
        self.assertIn('input.min="3"', source)
        self.assertIn('scrollHeight', source)
